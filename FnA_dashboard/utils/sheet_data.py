...
class SheetData(object):
    ...
    HEADERS = {
        ...
        "R-3-A": {
            "keys" : ["POLed.[Part]", "CAST(POLed.[Desc] AS NVARCHAR(100))","POLed.[Quan]","POLed.[Received]", "(POLed.[Quan] - POLed.[Received]) ","PO.[PO]","FORMAT (PO.Date, 'yyyy-MM-dd ')","FORMAT (PO.DateReq, 'yyyy-MM-dd ')", "PO.[ShipName]"],
        ...
        "R-3-D": {
            "keys" : ["PO.[PO]", "FORMAT (PO.[Date], 'yyyy-MM-dd ')","Vendor.[Company]","FORMAT (PO.DateReq, 'yyyy-MM-dd ')"],
            "headers" : ["PO","Date","Vendor", "Follow Up Date"]
        },
        "R-3-E": {
            "keys" : ["Vendor.[Company]", "VenPartsFirst.[Part]", "VenPartsFirst.[VenPart]", "VenPartsFirst.[LastPurchasePrice]", "FORMAT (VenPartsFirst.[DateLastPurchase], 'yyyy-MM-dd ')"],
        ...
        "R-3-G": {
            "keys" : ["Inven.[Part]","CAST(Inven.[Desc] AS NVARCHAR(100))","Inven.[Bprice]","Inven.[PriceA]","Inven.[PriceB]","Inven.[PriceC]"],
        ...
        "R-3-I": {
            "keys" : ["ViewListItems.[Item]","CAST(ViewListItems.[Description]  AS NVARCHAR(100))", "ViewListItems.[Vendor Company]", "ViewListItems.[Quantity in Stock]", "InvQuan.[WH]", "InvQuan.[Aisle]"],
            "headers" : ["Part","Description","Vendor Name", "Qty In Stock", "Warehouse", "Aisle"]
        },
        "R-3-J": {
            "keys" : ["InvenAct.[Part]", "MAX(CAST(InvenAct.[PartDesc] AS NVARCHAR(100)))","MAX(InvenAct.[WH])", "SUM(InvenAct.[Quan])", "MIN(InvenAct.Cost)","MAX(InvenAct.Cost)"],
